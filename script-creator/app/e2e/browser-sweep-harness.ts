...
export interface BrowserSweepLearningContext {
  page: Page;
  cloneRoot: string;
  cloneServer: string;
  runtimeFile: string;
...
export interface BrowserSweepHooks {
  afterTopicSelection?: (page: Page) => Promise<void>;
  afterArchitectureReview?: (
    architecturePanel: Locator,
  ) => Promise<void>;
  afterEpisodeAccepted?: (context: {
    page: Page;
    editorHost: Locator;
    editor: Locator;
  }) => Promise<void>;
  runLearningSweep?: (
    context: BrowserSweepLearningContext,
  ) => Promise<string>;
}

async function main(
  plan: 6 | 7,
  hooks: BrowserSweepHooks,
): Promise<void> {
  ...
  temporaryRoot = mkdtempSync(join(
    tmpdir(),
    plan === 7 ? 'whp-plan7-browser-' : 'whp-plan6-browser-',
  ));
  ...
  run('git', [
    ...
    'config',
    'user.name',
    plan === 7 ? 'Plan 7 Sweep' : 'Plan 6 Sweep',
  ]);
  run('git', [
    '-C',
    cloneRoot,
  ...
  await hooks.afterTopicSelection?.(page);
  ...
  await hooks.afterArchitectureReview?.(architecturePanel);
  ...
  await hooks.afterEpisodeAccepted?.({ page, editorHost, editor });
  ...
  if (hooks.runLearningSweep) {
    plan7ReconcileCommit = await hooks.runLearningSweep({
      page,
      cloneRoot,
      cloneServer,
      runtimeFile,
  ...
  assert(
    commitMessage === (plan7ReconcileCommit !== null
...
export async function runLearningLifecycleSweep(
  options: BrowserSweepLearningContext,
): Promise<string> {
  const {
    page,
    cloneRoot,
  ...
  const reconciliationToken = prepared.reconciliation?.resumeKey ?? '';
  assert(
    reconciliationToken !== ''
      && prepared.reconciliation?.preparedMarkdown.includes(
        `Reconciliation: ${reconciliationToken}`,
      ),
    'durable handoff omitted its exact reconciliation token',
  );
  assertCommitCount(
    cloneRoot,
    initialCommitCount,
...
export async function selectEditorParagraph(
  page: Page,
  editor: Locator,
  index: number,
): Promise<void> {
...
export async function waitForEditorSave(page: Page): Promise<void> {
...
export async function waitForMinimumCount(
  locator: Locator,
  minimum: number,
): Promise<void> {
  await waitForValue(
...
export async function runBrowserSweep(
  plan: 6 | 7,
  hooks: BrowserSweepHooks = {},
): Promise<void> {
  let failure: unknown = null;
  try {
    await main(plan, hooks);
  } catch (error) {
    failure = error;
  } finally {
    try {
  ...
  if (failure) {
    console.error(
      `FAILED — Plan ${plan} browser sweep`,
    );
    console.error(
    ...
    console.log(
      `VERIFIED — Plan ${plan} browser sweep`,
    );
  }
}
