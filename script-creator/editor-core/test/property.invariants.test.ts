...
class PropertyHarness {
  ...
  private insertOutside(index: number, text: string): void {
    ...
    if (pos !== undefined) this.apply(this.state.tr.insertText(text, pos), 'other');
  }
  ...
  private deleteSpan(
    ...
    const sameZone = spans.filter((span) =>
      span.from >= start.from && span.from < start.from + length);
    const last = sameZone.at(-1) ?? start;
    const before = this.state.doc;
  ...
  private assertExportInvariant(): void {
    ...
    for (const losing of getParkingLot(this.state)) {
  ...
  for (const seed of [20260722, 20260723]) {
    it(`preserves I1-I5 across generated operations (seed ${seed})`, () => {
      fc.assert(fc.property(
        ...
        (operations) => {
          const harness = new PropertyHarness();
          harness.assertInvariants();
          for (const operation of operations) harness.run(operation);
        },
      ), { numRuns: 250, seed });
    });
  }
