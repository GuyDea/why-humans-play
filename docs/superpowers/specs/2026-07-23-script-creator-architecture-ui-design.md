...
- Stored on the draft: `architecture: { sections: [{ key, title, md }], approvedMd:
  string | null, approvedAt: string | null }` in the existing drafts store (migration
  v4); revisions of the architecture append to the existing revision history with a
