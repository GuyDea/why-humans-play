...
**Contract:** toolbar appears on non-empty selection (plain DOM positioning like the Spike 2 demo); actions: Review, Rewrite, Alternatives (count 2–3), Custom instruction, Lock, Annotate, Flag-for-evidence (annotation kind evidenceFlag). Launch flow per the bridge decision table (spec'd verbatim as a pure function): rewrite → editor-core `requestProposal` at submit + `receiveProposal` on schema result (replacement_markdown) → ProposalLayer accept/reject; re-roll → daemon resume + a fresh receiveProposal on the SAME target (previous proposal rejected first); alternatives → on result, `insertVariantSet` at the selection with options from the labeled markdown list; review → `addAnnotation` per finding anchored by locating each finding's `anchor` text within the selection (fallback: selection range), findings panel lists them; guardrail → callout only, document untouched. Conflicted proposals surface base/current/proposed exactly as editor-core reports.
