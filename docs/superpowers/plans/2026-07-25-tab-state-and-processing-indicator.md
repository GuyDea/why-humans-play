...
- **Masthead-latency for page-local trackers.** Discover uses a page-local `OpTracker` (not session-attached), so its `ideate` op is only discovered by `ActiveOperationsService`'s server poll (~5s). Correction (post-implementation review): the inline Discover chip reads the same `activeOperations()`, so it also lags by up to one poll — it does **not** give instant feedback; the instant cue on Discover is the button's own "Suggesting…" label. Session-attached surfaces (editor, Topics) appear in the masthead immediately. Consequence: within ~5s the poll also *adopts* the Discover op into the hydration tracker, briefly double-streaming it (harmless — read-only, both settle, the chip dedupes by id). Attaching the Discover tracker via `attachRuntime` (like editor/Topics) would remove both the lag and the double-stream — a possible follow-up, out of scope for V1.
